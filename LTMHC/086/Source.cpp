...
int main()
{
	...
	cout << "\nSo luong gia tri chan nho nhat trong ma tran duoc tao: " << DemChanNhoNhat(a, m, n);
	return 0;
}
...
int ChanNhoNhat(int a[][500], int m, int n)
{
	int lc = ChanDau(a, m, n);
	for (int i = 0; i < m; ++i)
		for (int j = 0; j < n; ++j)
			if (a[i][j] % 2 == 0 && lc > a[i][j])
				lc = a[i][j];
	return lc;
}
